...
Flat marketing homepage only. Header/footer stay in `BaseLayout` (driven by
`navigation.ts` / `site.ts`). The page itself is assembled in `src/pages/index.astro`
from props-only blocks — no CMS, no Elementor parity layer.
...
| 1   | Hero (“Reveal the Beautiful Smile…”) + location pills + CTAs             | `Hero`                                                          | Copy hardcoded; pills from `locations` collection                      | **Done in starter shape.** Keep `background="inverse"` + `hero-bg.jpg`. Accent span on “Beautiful Smile” via title HTML.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 1b  | Trust chips under hero (Board Certified / In-Network / Free Consults)    | Inline list on `Hero` **or** thin `LogoCloud`-style caption row | Hardcoded                                                              | Live site shows icon chips. Prefer extending `Hero` with optional `trustItems` later; v1 can omit or put three short bullets on `Hero`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
```
BaseLayout
  Hero                    # 1 — inverse + bg image + pills + CTAs
